...
namespace ChessLibrary
{
    public class KingBehavior : Behavior
    {
        public override List<Point> GetAllMoves(ChessBoard board, Point location, bool pieceColor)
        {
            ...
            if (location.Y < board.MaxColumn - 1)
            {
                Point newLocation =  new Point(location.X, location.Y + 1);
                if (CanMove(board, newLocation))
                {
            ...
            if (location.X < board.MaxRow-1 && location.Y < board.MaxColumn - 1)
            {
                Point newLocation = new Point(location.X + 1, location.Y + 1);
                if (CanMove(board, newLocation))
                {
                    moves.Add(newLocation);
                }
            ...
            if (location.X < board.MaxRow-1 && location.Y > 0)
            {
                Point newLocation = new Point(location.X + 1, location.Y - 1);
                if (CanMove(board, newLocation))
                {
            ...
            if (location.X > 0 && location.Y < board.MaxColumn-1)
            {
                Point newLocation = new Point(location.X - 1, location.Y + 1);
                if (CanMove(board, newLocation))
                {
